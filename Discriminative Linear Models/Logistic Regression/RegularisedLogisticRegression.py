...
def cross_entropy(Y, predictions, theta0, theta1):
    reg_term = lambda_reg * (theta0**2 + theta1**2) / 2  # L2 regularization term
    return sum((Y[i]*math.log(predictions[i] + epsilon ) + (1-Y[i])*math.log(1-predictions[i] + epsilon )) for i in range(len(Y))) + reg_term


def logistic_gradient_ascent(theta0, theta1, features , target_values , learning_rate):
    logistic_features = [(1/(1+math.exp(-(theta0 + theta1*i)))) for i in features]
    m = len(target_values)

    gradient_theta0 = sum((target_values[i] - logistic_features[i]) for i in range(m)) + lambda_reg * theta0  # Added regularization term
    gradient_theta1 = sum((target_values[i] - logistic_features[i])* features[i] for i in range(m)) + lambda_reg * theta1  # Added regularization term
...
for i in range(num_iterations):
    ...
    cost = cross_entropy(y,predictions, theta0_log, theta1_log)
