...
def MSE(Y, predictions, theta0, theta1):
    reg_term = lambda_reg * (theta0**2 + theta1**2) / 2  # L2 regularization term
    return 0.5 * sum(((predictions[i] - Y[i])**2) for i in range(len(Y))) + reg_term
...
def batch_gradient_descent(theta0, theta1, features , target_values , learning_rate):
    ...
    gradient_theta0 = 0.5 * sum((theta0 + theta1 * features[i] - target_values[i]) for i in range(m)) + lambda_reg * theta0  # Added regularization term
    gradient_theta1 = 0.5 * sum((theta0 + theta1 * features[i] - target_values[i]) * features[i] for i in range(m)) + lambda_reg * theta1  # Added regularization term
...
def stochastic_gradient_descent(theta0, theta1, features , target_values , learning_rate, i):
    gradient_theta0 = 0.5 * (theta0 + theta1 * features[i] - target_values[i]) + lambda_reg * theta0  # Added regularization term
    gradient_theta1 = 0.5 * (theta0 + theta1 * features[i] - target_values[i]) * features[i] + lambda_reg * theta1  # Added regularization term
...
for i in range(num_iterations):
    ...
    cost = MSE(y,predictions,theta0_batch, theta1_batch)
...
for i in range(num_iterations):
    ...
    for j in range(m):
        ...
        cost = MSE(y,predictions,theta0_stochastic,theta1_stochastic)
